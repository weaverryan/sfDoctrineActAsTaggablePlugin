...
class TaggableListener extends Doctrine_Record_Listener
{
    ...
    */
    public function postSave(Doctrine_Event $event)
    {
        ...
        $taggings = $object->Tagging;
        $taggingComponentType = $taggings->getTable()->getComponentName();
        ...
        if($removed_tags)
        {
          $rm_tags = Doctrine::getTable('Tag')->createQuery('t INDEXBY t.id')->whereIn('t.name', $removed_tags)->fetchArray();
          $q = Doctrine_Query::create()
              ->delete()
              ->from('Tagging tg')
              ->whereIn('tg.tag_id', array_keys($rm_tags)) //$rm_tags->getPrimaryKeys()
              ->addWhere('tg.taggable_model = ?', get_class($object));
				  $removed_taggings = $q->execute();

          /* complains about constraint on tag_id
          $q = Doctrine_Query::create()
...
{    
  ...
  /*
    ...
    */
    ...
    {
//      $taggableClassname = $this->getTable()->getComponentName();
//      $taggable = new TaggableBehavior(array('className'=>$taggableClassname));

//      $tagging = Doctrine::getTable('Tagging');
//      $tagging->addGenerator($taggable, $taggableClassname);
//      $taggable->initialize($this->getTable());
...
//      $this->_plugin->initialize($this->_table);
    ...
    */
    ...
    public function addTagging(Tag $tag) {
      $tagging = new Tagging();
      $tagging->Tag = $tag;
    ...
    */
    ...
    public function getTagsTriple($ns, $key, $serialized = true, $return = 'tag') {
      $options = array('is_triple'=>true,'namespace'=>$ns, 'key'=>$key, 'serialized'=>$serialized, 'return'=>$return);
